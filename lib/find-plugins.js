...
async function findPlugins (dir, options) {
  const { opts, prefix, depth = 0, hooks = [] } = options

  const pluginTree = {
    [prefix || '/']: { hooks: [], plugins: [] }
  }

  await buildTree(pluginTree, dir, { prefix, opts, depth, hooks })
...
async function buildTree (pluginTree, dir, { prefix, opts, depth, hooks }) {
  ...
  const indexDirEntry = processIndexDirEntryIfExists(pluginTree, { dirEntries, opts, dir, prefix })
...
function processIndexDirEntryIfExists (pluginTree, { opts, dirEntries, dir, prefix }) {
  ...
  const hasDirectory = dirEntries.find((dirEntry) => dirEntry.isDirectory())
  if (!hasDirectory) {
    return pluginTree
  }

  return indexDirEntry
}
