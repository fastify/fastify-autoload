...
async function processList ({ list, opts, indexDirent, prefix, dir, depth, currentHooks, hookedAccumulator }) {
  for (const dirent of list) {
    ...
    if (dirent.isDirectory() && !atMaxDepth) {
      await processDirectory({ prefix, opts, dirent, dir, file, hookedAccumulator, depth, currentHooks })
    } else if (indexDirent) {
...
async function processDirectory ({ prefix, opts, dirent, dir, file, hookedAccumulator, depth, currentHooks }) {
  ...
  const hooks = opts.autoHooks && opts.cascadeHooks ? currentHooks : []
  await findPlugins(file, { opts, hookedAccumulator, prefix: prefixBreadCrumb, depth: depth + 1, hooks })
}
